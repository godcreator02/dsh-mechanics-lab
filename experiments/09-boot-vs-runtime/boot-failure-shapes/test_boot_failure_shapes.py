...
- **启动失败是走正常卸载流程退出的，不是硬崩。** 所以采集器来得及把事件刷完——
  失败现场的事件流是完整的，可以拿来排查。这条决定了「启动失败之后还能不能信
  事件流」，答案是能。
  ⚠️ 证据来自重组前那一轮实验的观测，**本项的用例没有正面断言它**（用例只做单向
  断言，见下）。要坐实得专门写一条：比对失败实例的事件流末尾与正常退出的实例。
...
- 「实例会死」是预期会发生的事，用轮询等（`while inst.alive(): sleep`），
  不用死等满超时。
- 事件流只做**单向**断言（「没出现过」），不断言「出现了几条」。这是防御性的：
  事件流的完整性不是本项的实验对象，拿它当强判据会让用例因为少收几条事件而假失败。
  ⚠️ 理由**不是**「进程崩着退所以事件丢了」——恰恰相反，见下面那条判定。
