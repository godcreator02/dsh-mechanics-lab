...
- **一个名字只有一个主人，谁先到谁得。** 两个插件占同一个名字，后到的那个当场
  失败，框架报的原话是 `service "labSeat" has been registered at <Include>`。
  失败者的 fiber 走 `LOADING → UNLOADING → FAILED`，`apply` 里 `provide` 之后的
  代码一行都没执行。已验：`test_two_bare_owners_collide`
- **独占不是服务基类那种写法的特性。** 裸对象 vs 服务基类抢同一个名字，报错文本
  与上一条**完全相同**——两种写法最终都落到框架那唯一一条注册通道上，重复检查
  长在那条通道里。已验：`test_bare_and_class_collide_too`
- **名字不同就不撞。** 同样两个插件、同样两种写法，只把名字改成不一样的：两个
  都占上、都跑到 `ACTIVE`，实例照常活着。撞车的病因是**名字相同**，不是「有两个
  提供者」，也不是「两种写法混用」。已验：`test_different_names_both_live`
- **启动期撞车拖垮整个实例**，退出码 1。连占名成功的那个也会被拆掉——事件流里
  它走完 `LOADING → ACTIVE` 之后还有 `UNLOADING → DISPOSED`，服务跟着撤销。

  `docs/official/zh/glossary.md:9` 把 Service Definition 定义为「拥有自身 `ctx.<key>`
  和词汇类型的 Cordis `Service`——绝不是 TypeScript `interface`」，`architecture.md:104`
  给出三角色的划分，两处都没有讲重复注册会怎样，本项实测补上。

## 观测方法

见证文件（只在占名成功之后才写，占没占上直接看它在不在）与事件流（fiber 状态、
服务出现/撤销、退出码）配合判定。日志里点名撞车的那几句（`has been registered` /
`did not activate` / `failed to load`）去重保序后作为判词呈现。

排错时用得上的两件事：**报错里点得出「谁失败了」，点不出「谁先占的」**——失败方
的条目 id 和 `name` 都在完整那句报错里，照着能在 patch 文件里找到那一行；先占者
只显示成 `<Include>`（fiber 名，不是条目 id），想知道先占者得去事件流里找那条
「服务出现」。

上一项 `provide` 立住两种提供写法。下一项 `registry` 从这里的铁律出发，问「想让
多个实现并存该怎么办」——答案不是打破这条铁律，是绕开它。
"""
