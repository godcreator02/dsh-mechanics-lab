"""availability-contract · 使用方能不能跑起来，谁说了算

档次 ③ ｜ 性质 🔬 ｜ 状态 ✅ ｜ 3 条用例 ｜ 不需要 web

## 判定

- **判据说了算，不是安排方式说了算。** 严格名册（写了 `[Service.check]()`）
  表空时，使用方卡在 PENDING、`apply` 一次没跑，整个实例起不来（退出码 1）；
  表里有一行时使用方正常跑到 `ACTIVE`。同一个表——都是空的——把名册换成没写
  这个判据方法的宽松版，使用方照样跑到 `ACTIVE`，拿到一张空表。「独占还是
  注册表」不决定使用方的命运，那一刻的可用性判据才决定。已验：
  `test_strict_and_empty_kills_the_instance` / `test_strict_with_rows_runs_fine` /
  `test_loose_and_empty_runs_fine`
- **判据否决造成的 PENDING，日志报不出在等谁。** 两种 PENDING 的判词不一样，
  而且是往难查的方向不一样：压根没人提供这个名字时判词点得出名字
  （`pending (waiting for service: labSomething)`）；有人占着、判据说不可用时
  只报 `pending (waiting for services: unknown)`——名字确实被占着，启动期审计
  那一侧看不出缺的是谁。看到 `waiting for services: unknown` 时，方向不是「去找
  谁没提供这个服务」，而是「去找哪个服务的可用性判据正在返回否」。已验：
  `test_strict_and_empty_kills_the_instance`

  判词前半截是条目的 `name`（如 `./roster-user.mjs`）而不是它的 id，排错时按
  `name` 去 patch 文件里找那一行。

## 观测方法

`provide` / `one-owner` / `registry` 三项读下来容易得出一个印象：独占式的提供方
缺席，使用方卡住、整个实例陪葬；注册表式的表空着，使用方照样活。好像命运由
「怎么安排多个实现」决定。这一项拆穿它：三个用例共用同一个使用方文件、一个字
都不改，换的只是对面那份名册；而两份名册之间只差一个方法
（`[Service.check]() { return this.表.size > 0 }`）——框架在决定「等着这个服务
的插件能不能开跑」之前会问一次，答否就当它不在。

日志里 `pending (waiting` / `did not activate` 两类判词去重保序后作为判据；
名册与使用方各自的 fiber 状态序列交叉核实「卡住的不是名册没起来」。

## 没覆盖到的

只测了**静态**场景：名册开张时表里有没有东西，判据在那一刻给什么答案。「表在
运行期从空变成非空，等着的插件会不会被唤醒」不在本项装置的覆盖范围内。

`registry` 立住了绕开独占的办法；本项是这条论证链的收口——拆穿「命运由安排
方式决定」这个印象，指出真正说了算的是可用性判据，而判据两种安排方式都能挂。
"""
...
def test_loose_and_empty_runs_fine(lab_home: LabHome, launch):
    ...
    由**那一刻的可用性判据**决定。`provide` / `one-owner` / `registry` 三项看到
    的差别，根子在这儿。
    """
