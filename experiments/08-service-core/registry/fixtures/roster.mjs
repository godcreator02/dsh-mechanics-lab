 ...
 *
 * `one-owner` 立住了「一个名字一个主人」。想让多个实现并存，办法不是去打破那条
 * 规矩，而是**别让它们争名字**：只有这个文件去占 `labRoster`，别人往它的表里塞。
 *
 * 注意 `register()` 里那句 `this.ctx.effect(...)`：
 * `provide` 那一项测出来，服务方法里的 `this.ctx` 属于**调用方**，所以这个 effect
 * 挂在塞东西的那个插件身上——它下线，它那一行自己就没了，不用名册去管。
 */
