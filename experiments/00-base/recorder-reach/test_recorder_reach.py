"""recorder-reach · 采集器（lab-recorder）自己是什么、能看见什么

档次 ① ｜ 性质 🔬 ｜ 状态 ✅ ｜ 3 条用例 ｜ 不需要 web

## 判定

- **它听得见别人，不止自己。** 同一次运行里，事件流除了它自己和旁边那条陪衬
  插件，还出现了二十多个从未写进 patch 文件的框架自带条目（含 `timer`、`hmr`）。
  用例：`test_it_hears_everyone_not_just_itself`
- **它自己也是树上的普通条目，没有特权。** 起点快照里有它自己一条，且那一刻
  它是 `LOADING`——快照正是在它自己的 `apply` 里拍的；它随后自己走到 `ACTIVE`，
  这一跳也被它自己记了下来。用例：`test_it_is_an_ordinary_entry`
- **它挂载之前的历史是丢的，不是没发生。** 事件流第一条是「采集开始」，紧接着
  是起点快照：睁眼那一刻已经在 `LOADING` 的条目，进 `LOADING` 那一跳没人看见，
  它自己就是这样的条目之一；睁眼后才建起来的条目整条链一跳不缺（对照组）。
  用例：`test_it_cannot_see_anything_before_it_opened_its_eyes`

## 观测方法

读 `observatory/lab-recorder` 写出的 `events.jsonl`。三条判定读**同一次运行**——
它们讲的是同一台仪器的三个侧面，分开跑要解释为什么两次数不一样。等待策略用
三条硬信号（起点快照拍完、陪衬插件说了话、陪衬插件到 `ACTIVE`），不是固定窗口
也不是单纯事件条数——后两者都会因框架自带条目数量浮动而不稳。

后面每一项的判定都由这台仪器给结论，这三条判定因此是所有观测数据的地基：
仪器要是只听得见自己那一摊，后面每一项的数据都只能说明「采集器怎么样」，
说明不了「实例里发生了什么」。
"""
