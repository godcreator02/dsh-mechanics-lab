...
- **最小插件集合是空集**。插件系统的基础设施是框架自带的，`dsh-base`（78 个条目）
  提供的是「一个 AI 助手」不是「一个插件运行时」，整个可选
...
**本课真正剩下的新增要测**：五层叠加里 `--patch` overlay 与 home 层的**相对顺序**
在 profile-boot 的 `composeLive()` 里是写死的（bundle 层 → profile 活层 → home 层
→ overlays），要验它跟实际生效顺序一致。
