...
**规格正本是 page-writing 篇**（`docs/page-writing.md`）：读者定位、四层内容、
正向在前与依赖只能向前、术语定义写法、用语、格式、覆盖程度怎么标，全在那里。
派 agent 时让它整篇照做，本计划不复述。

计划这边只管两件正本管不着的事：分批顺序，和每一项的图型判断。
...
规格正本是 page-writing 篇，本附录不复述。**实现直接抄 `00-base` 任一已完成页**
——那五页的 `.basemap` 是跑通并验过的，比另写一份骨架可靠。

抄的时候注意两件事：

- **树里的每一行都要对着本项自己的 `results/*/profiles/` 与 `fixtures/` 重列**，
  不要把被抄那页的文件名带过来。各项的 profile 名、插件名、有没有 home 级 patch 都不一样
- `--bmhit` 三处都要定义，取值别跟本页已有配色撞
