...
跳级会翻车——v1 的 E3 就是跳过「配方热重放」直奔「hmr 归属」才整批作废的：
...
分类依据是 `DRAFT.md` 第八节的三类盘点。**开课第一件事是查 `docs/official/`
...
- **每级先定观测方法，再写实验。** 观测信号选错会产出看似有效实则无意义的数据（L17 的
  前身 E3 就是这么废掉的）。
