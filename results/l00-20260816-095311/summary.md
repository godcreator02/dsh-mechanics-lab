...
── 显式禁用 hmr 之后的 settle 快照 ──
    ...
    · id=a1be1ad8                     @deepseek-ai/cordis-plugin-timer           state=2
    · id=169bbc99                     @deepseek-ai/cordis-plugin-hmr             state=2

  进程还活着？ True
  普查员被 apply 了 1 次
    plugin-timer：共 1 条，其中没被禁用的 1 条 → ['a1be1ad8']
    plugin-hmr：共 2 条，其中没被禁用的 1 条 → ['169bbc99']
...
── 连 timer 一起禁用 之后的 settle 快照 ──
    ...
    · id=my-hmr                       @deepseek-ai/cordis-plugin-hmr             无 fiber [disabled]
    · id=6f351344                     @deepseek-ai/cordis-plugin-timer           state=2
    · id=5c29f801                     @deepseek-ai/cordis-plugin-hmr             state=2

  进程还活着？ True
  普查员被 apply 了 1 次
    plugin-timer：共 2 条，其中没被禁用的 1 条 → ['6f351344']
    plugin-hmr：共 2 条，其中没被禁用的 1 条 → ['5c29f801']
  → 禁不掉。禁用只是让框架另造一份，服务照样在
```
