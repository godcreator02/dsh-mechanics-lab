...
跑于 2026-08-16 02:12:42（本地时间）

## 用例

### ✅ `test_entry_lands_in_composed_tree`  ·  0.63s
...
### ✅ `test_apply_actually_runs`  ·  3.74s

```
apply 执行于 2026-08-15T18:12:10.838Z，模块 import 于 2026-08-15T18:12:10.410Z
```

### ✅ `test_id_and_name_are_different_things`  ·  3.99s

```
id 与包名完全不同，插件照常加载 —— id 只是树内地址
```

### ✅ `test_config_reaches_apply`  ·  3.41s

```
apply 收到的 config：{'witness': 'D:/dshfiles/26081520anu/dsh-mechanics-lab/.testhome/l01/witness-config.json', '口令': '洛阳纸贵', '数字': 42, '开关': True}
```

### ✅ `test_resolution_is_the_real_boundary[exports]`  ·  3.72s

```
[exports 指向入口] → 加载了（符合预期）
```

### ✅ `test_resolution_is_the_real_boundary[index-fallback]`  ·  3.73s

```
[无 exports，入口恰好叫 index.js] → 加载了（符合预期）
```

### ✅ `test_resolution_is_the_real_boundary[no-entry-point]`  ·  12.07s

```
[无 exports，入口叫别的名字] → 没加载（符合预期）。实例还活着=False
```

### ✅ `test_resolution_is_the_real_boundary[main-fallback]`  ·  3.80s
