...
跑于 2026-08-16 10:08:45（本地时间）

## 用例

### ✅ `test_dependency_chain_loads_in_service_order`  ·  3.93s

```
账本顺序：['lab-alpha', 'lab-beta']
  alpha 登记时账本为空；beta 登记时已有 alpha，间隔 9.322ms
```
...
### ✅ `test_missing_middle_breaks_only_second_level`  ·  12.15s
...
### ✅ `test_entry_level_inject_vs_code_level`  ·  4.01s
...
### ✅ `test_write_order_does_not_decide_load_order`  ·  3.58s
...
```
lab-alpha 的全部事件：
    +   1.234ms  snapshot  fiberState=PENDING  hasFiber=True  disabled=False
    + 198.539ms  plugin  uid=None
    + 198.595ms    PENDING → UNLOADING
    + 202.101ms  UNLOADING → DISPOSED
...
[提供者被禁用] → **启动失败**（退出码 1）
    ...
    +   0.968ms  snapshot fiberState=PENDING
    + 206.903ms    PENDING → UNLOADING
    + 211.334ms  UNLOADING → DISPOSED
```
...
[服务名从不存在] → **启动失败**（退出码 1）
    ...
    [cause]: Error: dsh: 1 entry did not activate
    +   0.983ms  snapshot fiberState=PENDING
    + 189.984ms    PENDING → UNLOADING
    + 195.614ms  UNLOADING → DISPOSED
```

## 归档的观测产物

- `events-dis.jsonl` — 125,935 字节
- `events-never.jsonl` — 125,654 字节
- `events.jsonl` — 125,959 字节
