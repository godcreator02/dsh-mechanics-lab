...
── settle：静态构成 ──
    ...
    · id=58d0aa7a         @deepseek-ai/cordis-plugin-timer state=2
    · id=f9e46afb         @deepseek-ai/cordis-plugin-hmr   state=2
...
改动前 include.config.patches 里的 id：['census', 'profile-marker', 'home-marker']
  ...
  ── settle2：改动之后 ──
    ...
    · id=8a524c06         @deepseek-ai/cordis-plugin-timer state=2
    · id=e9813064         @deepseek-ai/cordis-plugin-hmr   state=2

  改动之后 include.config.patches 里的 id：['census', 'profile-marker', 'profile-marker-2', 'home-marker']
```

### ✅ `test_fallback_creates_even_when_disabled`  ·  2.57s
...
── 禁用之后 ──
    ...
    · id=home-marker      l07-marker-home                  无 fiber [disabled]  ⊂include
    · id=5e8c2bfd         @deepseek-ai/cordis-plugin-timer state=2
    · id=fa7a576b         @deepseek-ai/cordis-plugin-hmr   state=2

  hmr 条目：[('my-hmr', True), ('fa7a576b', False)]
```
...
### ✅ `test_ghost_ids_differ_names_stable`  ·  5.15s
...
── ghosta 的 settle 快照 ──
    ...
    · id=census           l07-census                       state=2  ⊂include
    · id=home-marker      l07-marker-home                  无 fiber [disabled]  ⊂include
    · id=7a0760e2         @deepseek-ai/cordis-plugin-timer state=2
    · id=ecbba6e1         @deepseek-ai/cordis-plugin-hmr   state=2

  ghosta 的幽灵条目：{'plugin-timer': '7a0760e2', 'plugin-hmr': 'ecbba6e1'}
  ...
  ── ghostb 的 settle 快照 ──
    ...
    · id=census           l07-census                       state=2  ⊂include
    · id=home-marker      l07-marker-home                  无 fiber [disabled]  ⊂include
    · id=c6ecd0ec         @deepseek-ai/cordis-plugin-timer state=2
    · id=e80f2925         @deepseek-ai/cordis-plugin-hmr   state=2

  ghostb 的幽灵条目：{'plugin-timer': 'c6ecd0ec', 'plugin-hmr': 'e80f2925'}

  → id 不稳定，只能认 name（包名后缀）
```

### ✅ `test_ghost_creation_does_not_lose_settle_snapshot`  ·  6.60s
