...
### ✅ `test_empty_bundles_boots`  ·  6.46s
...
普查员写出见证文件了吗？ True
  ...
  ── settle 快照 ──
    ...
    · id=9de499d5                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=d7a62ddb                 @deepseek-ai/cordis-plugin-hmr             state=2
```

### ✅ `test_ghost_entries`  ·  6.47s
...
配方里的条目（1 个）：['l00-census']
  ...
  ── settle：boot() 返回之后 ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=aad4228a                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=889548e3                 @deepseek-ai/cordis-plugin-hmr             state=2

  幽灵条目（树里有、配方里没有）：['@deepseek-ai/cordis-plugin-hmr', '@deepseek-ai/cordis-plugin-timer', 'cordis:include']
  boot 之后新增的条目 id：['889548e3', 'aad4228a']
```
...
── 显式禁用 hmr 之后的 settle 快照 ──
    ...
    · id=8d081341                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=c6610287                 @deepseek-ai/cordis-plugin-hmr             state=2

  进程还活着？ True
  普查员被 apply 了 1 次
    plugin-timer：共 1 条，其中没被禁用的 1 条 → ['8d081341']
    plugin-hmr：共 2 条，其中没被禁用的 1 条 → ['c6610287']
...
── 连 timer 一起禁用 之后的 settle 快照 ──
    ...
    ·     id=my-hmr                   @deepseek-ai/cordis-plugin-hmr             无 fiber [disabled]  ⊂include
    · id=8825c738                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=3cf9564e                 @deepseek-ai/cordis-plugin-hmr             state=2

  进程还活着？ True
  普查员被 apply 了 1 次
    plugin-timer：共 2 条，其中没被禁用的 1 条 → ['8825c738']
    plugin-hmr：共 2 条，其中没被禁用的 1 条 → ['3cf9564e']
  → 禁不掉。禁用只是让框架另造一份，服务照样在
```
...
timer 有没有被 link 进 profile？ False（预期 False）
  ...
  ── settle 快照 ──
    ...
    · id=5b067fda                 @deepseek-ai/cordis-plugin-hmr             state=2
...
看了 15 秒。进程还活着？ True
  ...
  ── 还活着时树里有什么 ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=45b1157f                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=b3ea865b                 @deepseek-ai/cordis-plugin-hmr             state=2
```
...
── 基线 · 默认（兜底） ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=3503fb14                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=1e868c69                 @deepseek-ai/cordis-plugin-hmr             state=2
    hmr 条目：['1e868c69']
...
### ✅ `test_pending_at_boot_is_fatal`  ·  0.60s
