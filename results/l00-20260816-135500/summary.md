...
启动后 cordis.yml 存在？ False
进程还活着？ True（退出码 None）
```

### ✅ `test_empty_bundles_boots`  ·  6.48s
...
普查员写出见证文件了吗？ True
  ...
  ── settle 快照 ──
    ...
    · id=8bed5acb                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=87bfc59b                 @deepseek-ai/cordis-plugin-hmr             state=2
```
...
配方里的条目（1 个）：['l00-census']
  ...
  ── settle：boot() 返回之后 ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=0836b727                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=dc3382d9                 @deepseek-ai/cordis-plugin-hmr             state=2

  幽灵条目（树里有、配方里没有）：['@deepseek-ai/cordis-plugin-hmr', '@deepseek-ai/cordis-plugin-timer', 'cordis:include']
  boot 之后新增的条目 id：['0836b727', 'dc3382d9']
```
...
── 显式禁用 hmr 之后的 settle 快照 ──
    ...
    · id=d56e304c                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=7b519674                 @deepseek-ai/cordis-plugin-hmr             state=2

  进程还活着？ True
  普查员被 apply 了 1 次
    plugin-timer：共 1 条，其中没被禁用的 1 条 → ['d56e304c']
    plugin-hmr：共 2 条，其中没被禁用的 1 条 → ['7b519674']
...
── 连 timer 一起禁用 之后的 settle 快照 ──
    服务：loader, timer, hmr
    · id=85124121                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    ·     id=my-timer                 @deepseek-ai/cordis-plugin-timer           无 fiber [disabled]  ⊂include
    ·     id=my-hmr                   @deepseek-ai/cordis-plugin-hmr             无 fiber [disabled]  ⊂include
    · id=40d32db4                 @deepseek-ai/cordis-plugin-hmr             state=2

  进程还活着？ True
  普查员被 apply 了 1 次
    plugin-timer：共 2 条，其中没被禁用的 1 条 → ['85124121']
    plugin-hmr：共 2 条，其中没被禁用的 1 条 → ['40d32db4']
  → 禁不掉。禁用只是让框架另造一份，服务照样在
```
...
timer 有没有被 link 进 profile？ False（预期 False）
  ...
  ── settle 快照 ──
    ...
    · id=a34f2ff5                 @deepseek-ai/cordis-plugin-hmr             state=2
...
看了 15 秒。进程还活着？ True
  ...
  ── 还活着时树里有什么 ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=f3eb4433                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=38cde318                 @deepseek-ai/cordis-plugin-hmr             state=2
```
...
── 基线 · 默认（兜底） ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=f6780e8b                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=ff791b57                 @deepseek-ai/cordis-plugin-hmr             state=2
    hmr 条目：['ff791b57']
...
### ✅ `test_pending_at_boot_is_fatal`  ·  0.59s
