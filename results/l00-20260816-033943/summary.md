...
跑于 2026-08-16 03:39:43（本地时间）

## 用例

### ✅ `test_profile_minimal_files`  ·  6.05s
...
普查员写出见证文件了吗？ True
  ...
  ── settle 快照 ──
    ...
    · id=db845552                     @deepseek-ai/cordis-plugin-timer           state=2
    · id=5addf7bf                     @deepseek-ai/cordis-plugin-hmr             state=2
```

### ✅ `test_ghost_entries`  ·  6.40s
...
配方里的条目（1 个）：['l00-census']
  ...
  ── settle：boot() 返回之后 ──
    ...
    · id=include                      cordis:include                             state=2
    · id=census                       l00-census                                 state=2
    · id=d4f5b537                     @deepseek-ai/cordis-plugin-timer           state=2
    · id=d39f39ea                     @deepseek-ai/cordis-plugin-hmr             state=2

  幽灵条目（树里有、配方里没有）：['@deepseek-ai/cordis-plugin-hmr', '@deepseek-ai/cordis-plugin-timer', 'cordis:include']
  boot 之后新增的条目 id：['d39f39ea', 'd4f5b537']
```
...
timer 有没有被 link 进 profile？ False（预期 False）
  ...
  ── settle 快照 ──
    ...
    · id=9108929a                     @deepseek-ai/cordis-plugin-hmr             state=2
...
### ✅ `test_relative_name_resolution[\u6307\u5411\u6587\u4ef6-/index.js]`  ·  6.05s
...
看了 15 秒。进程还活着？ True
  ...
  ── 还活着时树里有什么 ──
    ...
    · id=include                      cordis:include                             state=2
    · id=census                       l00-census                                 state=2
    · id=4347a5b9                     @deepseek-ai/cordis-plugin-timer           state=2
    · id=001a0782                     @deepseek-ai/cordis-plugin-hmr             state=2
```

### ✅ `test_baseline_profile`  ·  12.12s
...
── 基线 · 默认（兜底） ──
    ...
    · id=include                      cordis:include                             state=2
    · id=census                       l00-census                                 state=2
    · id=024e0cc5                     @deepseek-ai/cordis-plugin-timer           state=2
    · id=77e95c54                     @deepseek-ai/cordis-plugin-hmr             state=2
    hmr 条目：['77e95c54']
