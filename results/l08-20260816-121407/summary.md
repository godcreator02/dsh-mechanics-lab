...
[lab] 排队 46s 后拿到锁
  ...
  ── outer-group ⊃ {leaf-1, inner-group ⊃ leaf-2} ──
    ...
    · id=7df4fab5             @deepseek-ai/cordis-plugin-timer       state=2
    · id=e0f61c9c             @deepseek-ai/cordis-plugin-hmr         state=2

  深度：outer-group=1  leaf-1=2  inner-group=2  leaf-2=3
  扁平列表里的出场顺序：['include', 'census', 'outer-group', 'leaf-1', 'inner-group', 'leaf-2', '7df4fab5', 'e0f61c9c']
...
── group 自己不受 disabled 影响，孩子却因它而停 ──
    ...
    · id=99d5b28f             @deepseek-ai/cordis-plugin-timer       state=2
    · id=2e129ec5             @deepseek-ai/cordis-plugin-hmr         state=2
...
── 对照组：普通条目被禁用，自己就真的不激活 ──
    ...
    · id=27f60bbb             @deepseek-ai/cordis-plugin-timer       state=2
    · id=494bb5df             @deepseek-ai/cordis-plugin-hmr         state=2

  disabled-plain：disabled(原文)=True  hasFiber=False
```

### ✅ `test_isolate_gives_each_group_its_own_service_instance`  ·  3.67s
...
── group-a{flavor-a, taster-a} / group-b{flavor-b, taster-b} ──
    ...
    · id=b0273156             @deepseek-ai/cordis-plugin-timer       state=2
    · id=99f92e12             @deepseek-ai/cordis-plugin-hmr         state=2

  group-a 的 taster 看到：{'marker': 'lab-flavor-taster-v1', 'appliedAt': '2026-08-16T04:14:03.062Z', 'sawValue': 'vanilla', 'sawProviderMarker': 'lab-flavor-v1'}
  group-b 的 taster 看到：{'marker': 'lab-flavor-taster-v1', 'appliedAt': '2026-08-16T04:14:03.063Z', 'sawValue': 'chocolate', 'sawProviderMarker': 'lab-flavor-v1'}
