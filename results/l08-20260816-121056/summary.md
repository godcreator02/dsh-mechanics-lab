...
跑于 2026-08-16 12:10:56（本地时间）

## 用例

### ✅ `test_group_builds_nested_subtree`  ·  6.10s
...
── outer-group ⊃ {leaf-1, inner-group ⊃ leaf-2} ──
    ...
    · id=3cb12fa0             @deepseek-ai/cordis-plugin-timer       state=2
    · id=00c837d4             @deepseek-ai/cordis-plugin-hmr         state=2

  深度：outer-group=1  leaf-1=2  inner-group=2  leaf-2=3
  扁平列表里的出场顺序：['include', 'census', 'outer-group', 'leaf-1', 'inner-group', 'leaf-2', '3cb12fa0', '00c837d4']
  → 三层深的 leaf-2 和顶层的 census 摊在同一个列表里，没有 parent 字段分不出谁在谁下面
```

### ✅ `test_disabled_propagation[group \u81ea\u5df1\u4e0d\u53d7 disabled \u5f71\u54cd\uff0c\u5b69\u5b50\u5374\u56e0\u5b83\u800c\u505c-group-\n    - id: disabled-group\n      name: '@deepseek-ai/cordis-plugin-group'\n      group: true\n      disabled: true\n      config:\n        - id: child-under-disabled\n          name: l08-leaf\n-disabled-group-child-under-disabled]`  ·  6.10s
...
── 对照组：普通条目被禁用，自己就真的不激活 ──
    ...
    · id=b85f2b14             @deepseek-ai/cordis-plugin-timer       state=2
    · id=8ee68794             @deepseek-ai/cordis-plugin-hmr         state=2

  disabled-plain：disabled(原文)=True  hasFiber=False
```

### ✅ `test_isolate_gives_each_group_its_own_service_instance`  ·  3.15s

```
── group-a{flavor-a, taster-a} / group-b{flavor-b, taster-b} ──
    · id=61857800             @deepseek-ai/cordis-plugin-timer       state=2
    · id=include              cordis:include                         state=2
        · id=census               l08-census                             state=2  ⊂include
    ...
    · id=e6bb2614             @deepseek-ai/cordis-plugin-hmr         state=2

  group-a 的 taster 看到：{'marker': 'lab-flavor-taster-v1', 'appliedAt': '2026-08-16T04:10:53.050Z', 'sawValue': 'vanilla', 'sawProviderMarker': 'lab-flavor-v1'}
  group-b 的 taster 看到：{'marker': 'lab-flavor-taster-v1', 'appliedAt': '2026-08-16T04:10:53.050Z', 'sawValue': 'chocolate', 'sawProviderMarker': 'lab-flavor-v1'}
