...
### ✅ `test_empty_bundles_boots`  ·  6.49s
...
普查员写出见证文件了吗？ True
  ...
  ── settle 快照 ──
    ...
    · id=b642a1ba                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=bc02755d                 @deepseek-ai/cordis-plugin-hmr             state=2
```

### ✅ `test_ghost_entries`  ·  6.49s
...
配方里的条目（1 个）：['l00-census']
  ...
  ── settle：boot() 返回之后 ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=2d5c3bda                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=5f7b1a04                 @deepseek-ai/cordis-plugin-hmr             state=2

  幽灵条目（树里有、配方里没有）：['@deepseek-ai/cordis-plugin-hmr', '@deepseek-ai/cordis-plugin-timer', 'cordis:include']
  boot 之后新增的条目 id：['2d5c3bda', '5f7b1a04']
```
...
── 显式禁用 hmr 之后的 settle 快照 ──
    ...
    · id=fe30179c                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=74154c40                 @deepseek-ai/cordis-plugin-hmr             state=2

  进程还活着？ True
  普查员被 apply 了 1 次
    plugin-timer：共 1 条，其中没被禁用的 1 条 → ['fe30179c']
    plugin-hmr：共 2 条，其中没被禁用的 1 条 → ['74154c40']
...
── 连 timer 一起禁用 之后的 settle 快照 ──
    ...
    ·     id=my-hmr                   @deepseek-ai/cordis-plugin-hmr             无 fiber [disabled]  ⊂include
    · id=7cd43d3a                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=5613f691                 @deepseek-ai/cordis-plugin-hmr             state=2

  进程还活着？ True
  普查员被 apply 了 1 次
    plugin-timer：共 2 条，其中没被禁用的 1 条 → ['7cd43d3a']
    plugin-hmr：共 2 条，其中没被禁用的 1 条 → ['5613f691']
  → 禁不掉。禁用只是让框架另造一份，服务照样在
```

### ✅ `test_hmr_fallback_condition`  ·  6.08s
...
### ✅ `test_bare_name_resolution`  ·  6.09s
...
timer 有没有被 link 进 profile？ False（预期 False）
  ...
  ── settle 快照 ──
    ...
    · id=06373d4a                 @deepseek-ai/cordis-plugin-hmr             state=2
...
### ✅ `test_relative_name_resolution[\u6307\u5411\u76ee\u5f55-]`  ·  0.53s
...
看了 15 秒。进程还活着？ True
  ...
  ── 还活着时树里有什么 ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=f1efb1e2                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=2ae0b359                 @deepseek-ai/cordis-plugin-hmr             state=2
```

### ✅ `test_baseline_profile`  ·  12.17s
...
── 基线 · 默认（兜底） ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=2e85482b                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=6809f3d9                 @deepseek-ai/cordis-plugin-hmr             state=2
    hmr 条目：['6809f3d9']
