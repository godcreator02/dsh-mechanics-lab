...
跑于 2026-08-16 13:55:41（本地时间）

## 用例

### ✅ `test_entry_lands_in_composed_tree`  ·  0.55s
...
### ✅ `test_apply_actually_runs`  ·  12.13s

```
apply 执行于 2026-08-16T05:55:12.610Z，模块 import 于 2026-08-16T05:55:07.869Z
```

### ✅ `test_id_and_name_are_different_things`  ·  3.50s

```
id 与包名完全不同，插件照常加载 —— id 只是树内地址
```

### ✅ `test_config_reaches_apply`  ·  3.09s

```
apply 收到的 config：{'witness': 'D:/dshfiles/26081520anu/dsh-mechanics-lab/.testhome/l01/witness-config.json', '口令': '洛阳纸贵', '数字': 42, '开关': True}
```

### ✅ `test_resolution_is_the_real_boundary[exports]`  ·  3.18s

```
[exports 指向入口] → 加载了（符合预期）
```

### ✅ `test_resolution_is_the_real_boundary[index-fallback]`  ·  3.11s
...
### ✅ `test_resolution_is_the_real_boundary[main-fallback]`  ·  3.09s
