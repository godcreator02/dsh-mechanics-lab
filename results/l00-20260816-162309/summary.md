...
跑于 2026-08-16 16:23:09（本地时间）

## 用例

### ✅ `test_profile_minimal_files`  ·  2.06s
...
普查员写出见证文件了吗？ True
  ...
  ── settle 快照 ──
    ...
    · id=a0a459c5                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=cfda0b4b                 @deepseek-ai/cordis-plugin-hmr             state=2
```
...
配方里的条目（1 个）：['l00-census']
  ...
  ── settle：boot() 返回之后 ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=61dfe1d5                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=276f55b7                 @deepseek-ai/cordis-plugin-hmr             state=2

  幽灵条目（树里有、配方里没有）：['@deepseek-ai/cordis-plugin-hmr', '@deepseek-ai/cordis-plugin-timer', 'cordis:include']
  boot 之后新增的条目 id：['276f55b7', '61dfe1d5']
```
...
timer 有没有被 link 进 profile？ False（预期 False）
  ...
  ── settle 快照 ──
    ...
    · id=fb9c4992                 @deepseek-ai/cordis-plugin-hmr             state=2
...
### ✅ `test_who_keeps_process_alive`  ·  15.12s
...
看了 15 秒。进程还活着？ True
  ...
  ── 还活着时树里有什么 ──
    ...
    · id=include                  cordis:include                             state=2
    ·     id=census                   l00-census                                 state=2  ⊂include
    · id=754b1784                 @deepseek-ai/cordis-plugin-timer           state=2
    · id=edfdbf49                 @deepseek-ai/cordis-plugin-hmr             state=2
```

### ✅ `test_baseline_profile`  ·  12.19s
...
### ✅ `test_pending_at_boot_is_fatal`  ·  0.89s

```
进程还活着？ False（退出码 1，死于 +0.75s）
...
- `census-baseline-auto.json` — 2,765 字节
