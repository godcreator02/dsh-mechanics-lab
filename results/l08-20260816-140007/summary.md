...
── outer-group ⊃ {leaf-1, inner-group ⊃ leaf-2} ──
    ...
    · id=f43d5496             @deepseek-ai/cordis-plugin-timer       state=2
    · id=ec3bb80b             @deepseek-ai/cordis-plugin-hmr         state=2

  深度：outer-group=1  leaf-1=2  inner-group=2  leaf-2=3
  扁平列表里的出场顺序：['include', 'census', 'outer-group', 'leaf-1', 'inner-group', 'leaf-2', 'f43d5496', 'ec3bb80b']
...
── group 自己不受 disabled 影响，孩子却因它而停 ──
    ...
    · id=023ffdbb             @deepseek-ai/cordis-plugin-timer       state=2
    · id=93a09c4c             @deepseek-ai/cordis-plugin-hmr         state=2
...
── 对照组：普通条目被禁用，自己就真的不激活 ──
    ...
    · id=d9f71058             @deepseek-ai/cordis-plugin-timer       state=2
    · id=5db74058             @deepseek-ai/cordis-plugin-hmr         state=2

  disabled-plain：disabled(原文)=True  hasFiber=False
```

### ✅ `test_isolate_gives_each_group_its_own_service_instance`  ·  3.16s
...
── group-a{flavor-a, taster-a} / group-b{flavor-b, taster-b} ──
    ...
    · id=8a9ce5ac             @deepseek-ai/cordis-plugin-timer       state=2
    · id=b4f79153             @deepseek-ai/cordis-plugin-hmr         state=2

  group-a 的 taster 看到：{'marker': 'lab-flavor-taster-v1', 'appliedAt': '2026-08-16T06:00:03.845Z', 'sawValue': 'vanilla', 'sawProviderMarker': 'lab-flavor-v1'}
  group-b 的 taster 看到：{'marker': 'lab-flavor-taster-v1', 'appliedAt': '2026-08-16T06:00:03.845Z', 'sawValue': 'chocolate', 'sawProviderMarker': 'lab-flavor-v1'}
