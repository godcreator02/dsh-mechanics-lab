...
跑于 2026-08-16 09:57:40（本地时间）

## 用例

### ✅ `test_dependency_chain_loads_in_service_order`  ·  3.86s

```
账本顺序：['lab-alpha', 'lab-beta']
  alpha 登记时账本为空；beta 登记时已有 alpha，间隔 9.708ms
```
...
### ✅ `test_entry_level_inject_vs_code_level`  ·  3.59s
...
### ✅ `test_write_order_does_not_decide_load_order`  ·  3.54s
...
```
lab-alpha 的全部事件：
    +   2.089ms  snapshot  fiberState=PENDING  hasFiber=True  disabled=False
    + 215.861ms  plugin  uid=None
    + 215.907ms    PENDING → UNLOADING
    + 221.067ms  UNLOADING → DISPOSED

  labRegistry 从未出现在服务列表里（共 45 个服务）
```

### ✅ `test_boot_outcome_for_unsatisfiable_inject[provider-disabled]`  ·  6.19s
...
[提供者被禁用] → **启动失败**（退出码 1）
    ...
    +   1.015ms  snapshot fiberState=PENDING
    + 175.127ms    PENDING → UNLOADING
    + 178.669ms  UNLOADING → DISPOSED
```
...
[服务名从不存在] → **启动失败**（退出码 1）
    ...
    [cause]: Error: dsh: 1 entry did not activate
    +   0.973ms  snapshot fiberState=PENDING
    + 178.396ms    PENDING → UNLOADING
    + 182.597ms  UNLOADING → DISPOSED
```

## 归档的观测产物

- `events-dis.jsonl` — 125,927 字节
- `events-never.jsonl` — 125,666 字节
- `events.jsonl` — 125,964 字节
- `events-dis.relations.json` — 8,617 字节
- `events-never.relations.json` — 8,617 字节
- `events.relations.json` — 8,617 字节
- `ledger-inject.json` — 215 字节
