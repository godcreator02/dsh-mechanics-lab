...
跑于 2026-08-16 12:20:30（本地时间）

## 用例

### ✅ `test_insert_without_id_appends_to_root`  ·  0.51s
...
### ✅ `test_null_sets_key_to_null_not_delete`  ·  0.39s
...
### ✅ `test_later_patch_can_target_earlier_insert`  ·  0.41s
...
### ✅ `test_arbitrary_fields_pass_through_patch`  ·  0.38s
...
### ✅ `test_disabled_js_expression_is_evaluated[expr-false-loads]`  ·  2.00s

```
!!js false → 加载了：l05-patch-v1
```

### ✅ `test_disabled_js_expression_is_evaluated[expr-true-disabled]`  ·  13.37s

```
!!js true → 未加载（12s 内没写见证文件）
```

### ✅ `test_duplicate_id_at_boot_is_fatal`  ·  0.97s
